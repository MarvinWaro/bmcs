...
// Keep submission as YYYY-MM-DD with no TZ issues
...
export default function Welcome() {
  const { auth, errors, schools } = usePage<PageProps>().props;
  ...
  const [formData, setFormData] = useState({
    ...
    middleName: '',
    lastName: '',
    schoolId: '',              // <-- holds school id as string, or 'other'
    otherSchoolSpecify: '',
    transactionType: '',
  ...
  const toggleTheme = () => {
    switch (appearance) {
      ...
      default:       return updateAppearance('dark');
    }
  };
  const getThemeIcon = () => {
    switch (appearance) {
      ...
      Object.entries(errors).forEach(([field, message]) => {
        if (field === 'general') {
          ...
          const fieldLabels: Record<string, string> = {
            ...
            last_name: 'Last Name',
            email: 'Email Address',
            school_id: 'School/HEI',                // <-- updated key
          ...
          toast.error(`${fieldLabels[field] ?? field}: ${message}`);
        }
      });
    }
  }, [errors]);
  ...
  const handleRatingSelect = (rating: string) => {
    handleInputChange('satisfactionRating', rating);
    setCurrentStep(3);
  };

  // Date handlers (no TZ conversion)
  const handleDateChange = (date: Date | undefined) => {
    setSelectedDate(date);
    const display = date ? formatDate(date) : '';
    const submit = date ? formatDateForSubmission(date) : '';
    setDateValue(display);
    handleInputChange('transactionDate', submit);
  };
  const handleDateInputChange = (inputValue: string) => {
    ...
    if (isValidDate(parsed)) {
      ...
      setMonth(parsed);
      handleInputChange('transactionDate', formatDateForSubmission(parsed));
    } else {
  ...
  const handleSubmit = () => {
    ...
    const submitData = {
      ...
      // Backend expects either a valid school_id or "other_school_specify"
      school_id:
        formData.schoolId && formData.schoolId !== 'other'
          ? Number(formData.schoolId)
          : null,
      other_school_specify:
        formData.schoolId === 'other' ? formData.otherSchoolSpecify : '',
  ...
  const getReasonPrompt = () => {
    switch (formData.satisfactionRating) {
      case 'satisfied': return 'What made you satisfied with our service?';
      case 'dissatisfied': return 'What made you dissatisfied with our service?';
      default: return 'Please share your feedback:';
    }
  };
  ...
  const canProceedToRating = () => {
    ...
    // school must be chosen OR "other" specified
    const schoolOk =
      (formData.schoolId && formData.schoolId !== 'other') ||
      (formData.schoolId === 'other' && formData.otherSchoolSpecify.trim().length > 0);

    // transaction "other" requires specify
    const txnOk =
      formData.transactionType !== 'other' ||
      (formData.transactionType === 'other' && formData.otherTransactionSpecify.trim().length > 0);

    return hasBasics && schoolOk && txnOk;
  };

  // Display helpers
  const getSelectedSchoolName = () => {
    if (!formData.schoolId) return '';
    if (formData.schoolId === 'other') {
      return formData.otherSchoolSpecify || 'Other (Not Specified)';
    }
    const selected = schools.find(s => String(s.id) === String(formData.schoolId));
    return selected ? selected.name : formData.schoolId;
  };
  const getFullName = () => [formData.firstName, formData.middleName, formData.lastName].filter(Boolean).join(' ');
                      ...
                      <div className="space-y-2">
                        ...
                        <Select
                          ...
                          onValueChange={(value) => {
                            ...
                            if (value !== 'other') handleInputChange('otherSchoolSpecify', '');
                          }}
                        >
                      ...
                      <div className="space-y-2">
                        ...
                        <Select
                          ...
                          onValueChange={(value) => {
                            ...
                            if (value !== 'other') handleInputChange('otherTransactionSpecify', '');
                          }}
                        >
